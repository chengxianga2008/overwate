            ...
            if ($token) {
                ...
                if ($client->isAccessTokenExpired()) {
                    ...
                    if (!empty($refreshToken)) {
                        ...
                        try {
                            $oldAccessToken = json_decode(base64_decode($this->configuration->get('accessToken')), true);
                        ...
                        $this->configuration->set('accessToken', base64_encode(json_encode(array_merge($oldAccessToken, json_decode($client->getAccessToken(), true)))));
    ...
    public function finishAuth() {
        ...
        try {
            ...
            if (!$accessToken) {
                ...
                try {
                    $oldAccessToken = json_decode(base64_decode($data['accessToken']), true);
                    if (!is_array($oldAccessToken)) {
                        $oldAccessToken = array();
                    }
                } catch (Exception $e) {
                ...
                $data['accessToken'] = base64_encode(json_encode($newAccessToken));
                $this->addData($data);
