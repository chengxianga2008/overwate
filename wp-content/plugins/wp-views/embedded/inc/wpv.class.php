...
class WP_Views {
	...
	function __construct() {
		...
		add_filter( 'wpv_filter_wpv_get_editor_addon',					array( $this, 'get_editor_addon' ) );
	 ...
	 * get_editor_addon
	 *
	 * @usage apply_filters( 'wpv_filter_wpv_get_editor_addon', null );
	 *
	 * @since 2.3.0 Deprecated, but still used in the Loop Wizard :-/
	 */
	
	function get_editor_addon( $editor_addon = null ) {
		return $this->wpv_editor_addon;
	}
	...
	*/
	...
	function wpv_register_assets() {
		...
		global $pagenow, $post;
		wp_register_script( 'views-shortcodes-gui-script', WPV_URL_EMBEDDED . '/res/js/views_shortcodes_gui.js', array( 'jquery', 'suggest', 'jquery-ui-dialog', 'jquery-ui-tabs', 'views-utils-script', 'quicktags', 'shortcode', 'icl_editor-script', 'underscore', 'toolset-event-manager' ), WPV_VERSION );
...
function wpv_views_plugin_plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
	...
	if ( $plugin_file == $this_plugin ) {
		$plugin_meta[] = sprintf(
				...
				'https://wp-types.com/version/views-2-5/?utm_source=viewsplugin&utm_campaign=views&utm_medium=release-notes-plugin-row&utm_term=Views 2.5 release notes',
				__( 'Views 2.5 release notes', 'wpv-views' ) 
			);
	}
	return $plugin_meta;
}
